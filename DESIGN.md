...
- **What this app is:** A small self-hosted web app that answers "do I already have this book?" by querying Audiobookshelf and Calibre concurrently and displaying the result sets side-by-side, with deep link handoff to Shelfmark.
- **Who uses it:** Personal / self-hosted users managing media libraries behind an auth proxy.
- **Router / framework choice and why:** Vite + React SPA. Single-page search utility behind an auth proxy; no SSR, RSC, or SEO requirements.
- **Backend and where its OpenAPI spec lives:** FastAPI backend at `/api`; OpenAPI spec generated at `/openapi.json`.
- **Pagination convention:** Unpaginated; fixed `SEARCH_LIMIT=25` (configurable up to 200) per source.
- **Deviations from the shared conventions (with reasons):**
  - Date parsing: Only publication year string is received and displayed (`book.year`), so `date-fns` formatting is deferred.
  - JSON wire format: Backend models currently return snake_case fields (`shelfmark_url`, `cover_url`, `item_url`, etc.) rather than camelCase.
