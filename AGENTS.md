...
Hooks come from the shared `MartinCa/lefthook-configs` fragments pinned at `v1.0.1` in `lefthook.yml`. `remotes:` configs merge _over_ `lefthook.yml`. The fragments are native to this repo's tooling (pnpm on the frontend, uv/ruff on the backend), so the only `lefthook-local.yml` entries are `ts-lint`/`ts-format`: `langs/ts.yml` and `langs/python.yml` both name their commands `lint`/`format`, and when two `remotes:` configs define the same command the last one wins in the merge, so the python fragment would otherwise clobber the TS eslint/prettier commands. The local override reproduces them under distinct names.
